...
<head>
	...
	<title>Editar pregunta - RadTree</title>
	<meta name="description"
		content="Formulario para la edición de preguntas">
	<meta name="author"
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-md-12 col-lg-8">
					...
					<form name="editarPregunta" action="${pageContext.request.contextPath}/gestion-contenido/editar-pregunta/editar" method="post">

						<input type="hidden" name="id" value="<c:out value="${requestScope.pregunta.idContenido}"/>"/>
						
						<c:if test="${respuestasTotales == null}">
							<c:if test="${requestScope.respuestas == null}">
								<c:set var="respuestasTotales" scope="request" value="${2}"/>
							</c:if>
							<c:if test="${requestScope.respuestas != null}">
								<c:set var="respuestasTotales" scope="request" value="${requestScope.respuestas.size()}"/>
							</c:if>
						</c:if>
						...
						<div class="row form-block form-plr-15 form-h-45 form-mb-20 form-brdr-lite-white">
							...
							<div class="col-12">
								...
								<input class="mt-5" type="text" name="enunciado"
									...
									value="<c:out value="${requestScope.pregunta.enunciado}"/>" />
							</div>
							...
							<c:forEach var="i" begin="1" end="${respuestasTotales}">
								...
								<div class="pl-50 pl-sm-25 col-10">
									...
									<input type="text" name="<c:out value ="${resN}"/>"
										...
										value="<c:out value="${requestScope.respuestas[i - 1].enunciado}"/>" />
								</div>
							
								<div class="col-2">
									<input class="mt-30" type="checkbox" name="<c:out value="${correctaN}"/>"
										<c:if test="${requestScope.respuestas[i - 1].correcta}">checked</c:if>/>
								</div>
							</c:forEach>
						...
						<div class="row">
							<div class="col-12 mt-30">
								...
								<button class="w-100 btn-fill-primary" name="button" value="crearPregunta" type="submit"><b>Finalizar edición</b></button>
							</div>
						</div>
