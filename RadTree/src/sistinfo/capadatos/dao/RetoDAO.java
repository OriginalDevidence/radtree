...
import sistinfo.servlets.jsp.carteles.ListaDeRetosServlet.FiltroReto;
...
public class RetoDAO extends ContenidoDAO {
	...
	public List<RetoVO> getRetosByAutor(Long idAutor) throws ErrorInternoException {
		...
		try {
			...
			PreparedStatement stmt = connection
					.prepareStatement("SELECT * FROM Reto NATURAL JOIN Contenido WHERE idAutor=?");
	...
	/**
	 * Búsqueda de hasta num retos validados que contienen search en su título o
	 * cuerpo, ordenados por su fecha de realización
	 * 
	 * @param search
	 * @param num
	...
	public List<RetoVO> getRetosBySearch(String search, int num) throws ErrorInternoException {
		...
		try {
			...
			PreparedStatement stmt = connection.prepareStatement(
					"SELECT * FROM Reto NATURAL JOIN Contenido WHERE estado='VALIDADO' AND titulo LIKE ? OR cuerpo LIKE ? ORDER BY fechaRealizacion DESC");
	 ...
	 * @throws ErrorInternoException
	 */
	public List<RetoVO> getRetosBySearchByPag(String search, int retosPorPagina, FiltroReto filtro, int pagina)
			throws ErrorInternoException {
		List<RetoVO> listReto = new ArrayList<RetoVO>();
		try {
			Connection connection = ConnectionFactory.getConnection();
			...
			PreparedStatement stmt = connection
					.prepareStatement("SELECT * FROM Reto NATURAL JOIN Contenido WHERE estado='VALIDADO' AND "
							+ filtroString + "ORDER BY fechaRealizacion DESC");
			...
			rs.absolute(retosPorPagina * (pagina - 1));
			while (rs.next() && listReto.size() < retosPorPagina) {
				RetoVO reto = extractRetoFromResultSet(rs);
				listReto.add(reto);
			}
	 ...
	 * @throws ErrorInternoException
	 */
	public int getNumRetosBySearch(String search, int retosPorPagina, FiltroReto filtro)
			throws ErrorInternoException {
		int numRetos;
		...
		try {
			...
			// Crear el statement con los filtros
			PreparedStatement stmt = connection.prepareStatement(
					"SELECT COUNT(idContenido) AS numContenido FROM Reto NATURAL JOIN Contenido WHERE estado='VALIDADO' AND " + filtroString);
			for (int i = 0; i < numFiltros; i++) {
				stmt.setString(i + 1, "%" + search + "%");
			}
			...
			rs.first();
			numRetos = rs.getInt("numContenido");
			stmt.close();
			connection.close();
	...
	/**
	 * Búsqueda de hasta los últimos num retos validados ordenados por su fecha de
	 * realización
	 * 
	 * @param num
	...
	public List<RetoVO> getRetosUltimos(int num) throws ErrorInternoException {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(
					"SELECT * FROM Reto NATURAL JOIN Contenido WHERE estado='VALIDADO' ORDER BY fechaRealizacion DESC");
			while (rs.next() && listReto.size() < num) {
				RetoVO reto = extractRetoFromResultSet(rs);
				listReto.add(reto);
			}
	 ...
	 * @throws ErrorInternoException
	 */
	public List<RetoVO> getRetosUltimosByPag(int retosPorPagina, int pagina) throws ErrorInternoException {
		List<RetoVO> listReto = new ArrayList<RetoVO>();
		try {
			Connection connection = ConnectionFactory.getConnection();
	 ...
	 * @throws ErrorInternoException
	 */
	public int getNumRetosUltimos(int num) throws ErrorInternoException {
		int numRetos;
		try {
			Connection connection = ConnectionFactory.getConnection();

			Statement stmt = connection.createStatement();
			ResultSet rs = stmt.executeQuery("SELECT COUNT(idContenido) AS numContenido FROM Reto NATURAL JOIN Contenido WHERE estado='VALIDADO'");
	...
	public boolean updateReto(RetoVO reto) throws ErrorInternoException {
		try {
			...
			PreparedStatement stmt = connection
					.prepareStatement("UPDATE Reto SET titulo=?, cuerpo=? WHERE idContenido=?");
	...
	/**
	 * Extrae los datos de un reto dado un ResultSet. IMPORTANTE: El resultado de la
	 * consulta debe tener los atributos de Contenido además de Reto.
	 * 
	 * @param rs
	...
	private RetoVO extractRetoFromResultSet(ResultSet rs) throws SQLException {
		RetoVO reto = new RetoVO(rs.getLong("idContenido"), rs.getLong("idAutor"), rs.getLong("numVisitas"),
				rs.getDate("fechaRealizacion"), ContenidoVO.Estado.valueOf(rs.getString("estado")),
				rs.getString("titulo"), rs.getString("cuerpo"));
		return reto;
	}
